...
export default function Navbar({ setBillsToShow }: { setBillsToShow: (billsToShow: string) => void }) {
  ...
  const { getAccessToken, refreshAccessToken, setAccessToken } = useAuth();
  useEffect(() => {
    const getUserName = async () => {
      ...
      if (response.ok) {
        ...
        if (newToken) {
          getUserName(); // Retry fetching username with the new token
        } else {
  ...
  return(
    <nav className='md:h-full md:w-[20%] w-full p-2 bg-[#fcfaf8]'>
      ...
      <div className='flex md:flex-col flex-row flex-wrap items-center md:mt-10 gap-2'>
        <div className='flex flex-row justify-start items-center md:w-full hover:bg-[#dd493f22] p-2 rounded-md text-[#dd493f] font-medium z-20 cursor-pointer'
        ...
        <div className='flex flex-row justify-start items-center md:w-full hover:bg-[#dd493f22] p-2 rounded-lg hover:text-[#dd493f] cursor-pointer'
